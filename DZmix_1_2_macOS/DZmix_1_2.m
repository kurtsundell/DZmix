function varargout = DZmix_1_2(varargin)
%DZMIX_1_2 M-file for DZmix_1_2.fig
%      DZMIX_1_2, by itself, creates a new DZMIX_1_2 or raises the existing
%      singleton*.
%
%      H = DZMIX_1_2 returns the handle to a new DZMIX_1_2 or the handle to
%      the existing singleton*.
%
%      DZMIX_1_2('Property','Value',...) creates a new DZMIX_1_2 using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to DZmix_1_2_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      DZMIX_1_2('CALLBACK') and DZMIX_1_2('CALLBACK',hObject,...) call the
%      local function named CALLBACK in DZMIX_1_2.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help DZmix_1_2

% Last Modified by GUIDE v2.5 17-Nov-2018 15:42:46

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DZmix_1_2_OpeningFcn, ...
                   'gui_OutputFcn',  @DZmix_1_2_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before DZmix_1_2 is made visible.
function DZmix_1_2_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for DZmix_1_2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes DZmix_1_2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = DZmix_1_2_OutputFcn(hObject, eventdata, handles)
...
colorbar;
hCbar = colorbar;
set(hCbar, 'Ticks',(1/(N*2)):(1/(N)):1-(1/(N*2)))
hCbarLabels = {1:1:N};
    ...
    case handles.density_KDEs
        ...
        for i = 1:nsamples;
            ...
            if test==1;
                ...
                pdp_cdf_out(:,1) = x;
                pdps = pdp_out(:,2:end);
                assignin('base','bandwidth_write',round(10*mean(bandwidth_out))/10)
...
xlwrite([path file], Results_export);
