...
trials = 100; % Number of randomly generated sets of weights used to scale source distributions for comparison to mixed distribution
...
plotnum = 10; % number of best fits to retain (and plot) to calculate weight mean and standard dev. contribution from each source sample

%% OPTIONAL USER SPECIFICATIONS %%

% Randomly n ages resulting in N source distributions for each randomly generated set of weights during the Monte Carlo simulation. WARNING! Computationally intensive
Licht_approach = 1; % 0 = Do not use Licht et al. (2016) approach. 1 = Use approach during Monte Carlo model. 2 = Use approach following optimization (see below) 
Licht_N = 100; % N = the number of times each source distribution is subsampled (see Licht et al. (2016) for details, N = 200 in that paper)
Licht_n = 70; % n = the total number of ages sum of subsampled source ages; n must be <= minimum source sample size (see Licht et al. (2016) for details, n = 800 in that paper)

% Optimization options
Optimize = 1; % 0 = Do not run optimization routine. 1 = Run optimization routine. Optimization constrints set based on best-fit mean and standard dev. results from Monte Carlo model, see Sundell and Saylor (2017) for details
...
figure;
for i = 1:plotnum
plot(Results_D(i,N+3+length(Passed_cdf_D(:,1)):end),Results_D(i,N+3:N+2+length(Passed_cdf_D(:,1))), 'b', 'linewidth', 1)
hold on
end
...
figure;
for i = 1:plotnum
plot(Results_V(i,N+3+length(Passed_cdf_V(:,1)):end),Results_V(i,N+3:N+2+length(Passed_cdf_V(:,1))), 'b', 'linewidth', 1)
hold on
end
cdf_sink_ages = nonzeros(data(:,1));
cdf_sink_ages(isnan(cdf_sink_ages(:,1)),:) = [];
...
figure;
for i = 1:plotnum
plot(x,Results_R2(i,N+3:end), 'b', 'linewidth', 1)
hold on
end
...
line(Passed_D(j,1),Passed_weights_D(j,i),'LineStyle','none','Marker','o','MarkerFaceColor',colours((i),:),'MarkerEdgeColor','none',...
	'MarkerSize',6);
end
end
figure(KS_D)
...
line(Passed_V(j,1),Passed_weights_V(j,i),'LineStyle','none','Marker','o','MarkerFaceColor',colours((i),:),'MarkerEdgeColor','none',...
	'MarkerSize',6);
end
end
figure(Kuiper_V)
...
line(Passed_R2(j,1),Passed_weights_R2(j,i),'LineStyle','none','Marker','o','MarkerFaceColor',colours((i),:),'MarkerEdgeColor','none',...
	'MarkerSize',6);
end
end
figure(R2)
...
cdf_D_best(:,i) = sum(cdf_source.*repmat(D_conc_sort(i,2:end).*0.01,length(x_all),1),2);
end
figure;
hold on
p2 = plot(x_all,cdf_sink, 'k', 'linewidth', 2);
for i = 1:Max_best_fits
p1 = plot(x_all,cdf_D_best(:,i), 'm','linewidth',1);
end
hold off
axis([PDP_min PDP_max 0 1])
title('KS test D statistic')
...
for i = 1:Max_best_fits
cdf_V_best(:,i) = sum(cdf_source.*repmat(V_conc_sort(i,2:end).*0.01,length(x_all),1),2);
end
figure;
hold on
p4 = plot(x_all,cdf_sink, 'k', 'linewidth', 2);
for i = 1:Max_best_fits
p3 = plot(x_all,cdf_V_best(:,i), 'm','linewidth',1);
end
hold off
axis([PDP_min PDP_max 0 1])
...
for i = 1:Max_best_fits
pdp_R2_best(:,i) = sum(pdp_source.*repmat(R2_conc_sort(i,2:end).*0.01,length(x),1),2);
end
figure;
hold on
p6 = plot(x,pdp_sink, 'k', 'linewidth', 2);
for i = 1:Max_best_fits
p5 = plot(x,pdp_R2_best(:,i), 'm','linewidth',1);
end
hold off
axis([PDP_min PDP_max 0 max(pdp_sink)+max(pdp_sink)*.25])
...
line(D_conc_out(j,1),D_conc_out(j,i+1),'LineStyle','none','Marker','o','MarkerFaceColor',colours((i),:),'MarkerEdgeColor','none',...
	'MarkerSize',6);
end
end
figure(KS_D)
...
line(V_conc_out(j,1),V_conc_out(j,i+1),'LineStyle','none','Marker','o','MarkerFaceColor',colours((i),:),'MarkerEdgeColor','none',...
	'MarkerSize',6);
end
end
figure(Kuiper_V)
...
line(R2_conc_out(j,1),R2_conc_out(j,i+1),'LineStyle','none','Marker','o','MarkerFaceColor',colours((i),:),'MarkerEdgeColor','none',...
	'MarkerSize',6);
end
end
figure(R2)
